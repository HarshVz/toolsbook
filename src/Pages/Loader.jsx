...
const Loader = () => {
  ...
  return (
    <div className={`absolute inset-0 flex items-center justify-center z-60 min-h-screen h-full bg-zinc-900/80 w-full ${isLoading ? "visible" : "hidden"}`}>
...
/* From Uiverse.io by Javierrocadev */
        <div className='flex justify-center items-center w-full h-full flex-col gap-3'>
            <div class="flex flex-row gap-2 w-full justify-center items-center transition duration-300 ease-in">
  <div class="w-4 h-4 rounded-full bg-yellow-600 animate-bounce"></div>
  <div class="w-4 h-4 rounded-full bg-yellow-600 animate-bounce [animation-delay:-.3s]"></div>
  <div class="w-4 h-4 rounded-full bg-yellow-600 animate-bounce [animation-delay:-.5s]"></div>
</div>
{
 (
